function CliffWalkingSARSA(options)
% CliffWalkingSARSA: implements the cliff-walking problem using the
% SARSA method
...
for ei = 1:episodeCount,
    ...
    if(selectedEpIndex <= length(selectedEpisodes) && ei == selectedEpisodes(selectedEpIndex))
        ...
        if(showTitle == 1),
            title(sprintf('Cliff Walking SARSA - episode %d - (\\epsilon: %3.3f), (\\alpha = %3.4f), (\\gamma = %1.1f)', ei, epsilon, alpha, gamma));
        end
